...
## Verified causal hole in current code
...
However, before the mitigation there were two uncovered failure paths:
...
This is a real UI/lifecycle hole supported by source inspection. It does **not** prove that this was the exact hardware root cause previously observed.
...
5. No Wi-Fi ownership, provisioning state, Xiaozhi transport, cloud lifecycle, audio lifecycle, NVS data, or reset behavior is changed.

This is intentionally a **composition-layer fail-safe**, not a guessed repair to `wifi_manager` or `provisioning_manager`.
...
Confirm the new fallback does not activate.
...
### Test 4 — coordinator start failure

If a safe test configuration can make `app_network_coordinator_start()` return an error, expected marker:
...
The new BOOT guard must never override the provisioning screen.

### Test 6 — Xiaozhi feature gate comparison
...
Important network coordinator checkpoints:
...
- new BOOT-failure fallback markers listed above
