...
$bufferModule = Join-Path $componentRoot 'modules\buffer'
New-Item -ItemType Directory -Force -Path $outputRoot | Out-Null
$exe = Join-Path $outputRoot 'log_manager_tests.exe'
& gcc -std=c11 -Wall -Wextra -Werror -pthread -I "$testRoot/include" -I "$componentRoot/include" -I "$bufferModule/include" -I "$componentRoot/../app_log/include" "$testRoot/platform.c" "$bufferModule/src/log_buffer.c" "$componentRoot/../app_log/app_log.c" "$testRoot/test_log_manager.c" -o $exe
...
& gcc -std=c11 -Wall -Wextra -Werror -pthread -DCONFIG_LOG_MANAGER_ENABLE=0 -I "$testRoot/include" -I "$componentRoot/include" -I "$bufferModule/include" -I "$componentRoot/../app_log/include" "$testRoot/platform.c" "$bufferModule/src/log_buffer.c" "$componentRoot/../app_log/app_log.c" "$testRoot/test_disabled.c" -o $disabledExe
...
& gcc -std=c11 -Wall -Wextra -Werror -pthread -I "$testRoot/include" -I "$componentRoot/include" -I "$bufferModule/include" -I "$componentRoot/../app_log/include" "$testRoot/platform.c" "$bufferModule/src/log_buffer.c" "$componentRoot/../app_log/app_log.c" "$testRoot/test_durability.c" -o $durabilityExe
