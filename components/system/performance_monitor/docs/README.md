...
- Measures CPU busy and idle percentages from two FreeRTOS runtime snapshots.
...
```text
I (...) PERF_MONITOR: [CPU] used=12.3%, idle=87.7%, cores=2
...
- CPU usage is system-wide runtime utilization over a 5-second window. It is
  different from LVGL's optional performance overlay CPU estimate.
