 ...
 *
 * This file intentionally owns only the low-level synchronous WAV reader.
 * Two-slot PSRAM prefetch belongs exclusively to audio_wav_prefetch.c.
 */
    ...
    int error_number)
{
    ...
    /*
     * newlib/FatFs does not always preserve a useful errno after ferror().
     * A generic ESP_FAIL with errno==0 is therefore still a confirmed stream
     * I/O failure. Expected structural errors such as truncated data are not
     * reported through this helper.
     */
    if ((result == ESP_FAIL) &&
        ((error_number == 0) ||
         sd_card_manager_is_vfs_media_error(error_number)))
    {
...
bool audio_wav_path_is_valid(const char *path)
{
    ...
    return (path[mount_length] == '/') &&
           (path[mount_length + 1U] != '\0');
}
    ...
    audio_wav_info_t *info)
{
    ...
    *info = (audio_wav_info_t) {0};
    ...
    if (!found_format || !found_data)
    {
        /* Existing file, malformed RIFF/WAVE structure. */
        return ESP_ERR_INVALID_RESPONSE;
    }
    ...
    const char *path)
{
    ...
    if (result != ESP_OK)
    {
        ...
        if (parse_io_error)
        {
            audio_wav_report_media_error_if_needed(ESP_FAIL, parse_errno);
        }
    ...
    ESP_LOGI(
        ...
        (unsigned)AUDIO_WAV_STREAM_BUFFER_BYTES);
...
esp_err_t audio_wav_stream_read_limited(
    ...
    size_t *bytes_read)
{
    if ((stream == NULL) || (max_bytes == 0U) ||
        (buffer == NULL) || (bytes_read == NULL))
    {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    *bytes_read = 0U;
    ...
    /*
     * READY is the only state that accepts/keeps normal managed I/O. If another
     * consumer has already reported a media failure, recovery will have moved
     * away from READY and is waiting for this lease to drain.
     */
    ...
    uint64_t data_offset_bytes)
{
    ...
    if (result != ESP_OK)
    {
        ...
        audio_wav_report_media_error_if_needed(result, seek_errno);
        return result;
    }
