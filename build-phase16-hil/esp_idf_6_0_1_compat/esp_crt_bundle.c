...
#if defined(CONFIG_MBEDTLS_CERTIFICATE_BUNDLE_CROSS_SIGNED_VERIFY)
/* Reference ASN.1 named data by pointing into src's buffers.
...
static int esp_crt_ca_cb_callback(void *ctx, mbedtls_x509_crt const *child, mbedtls_x509_crt **candidate_cas)
{
    ...
    /* The certificate bundle remains alive for the handshake. */
    ...
    /* Populate parent subject by referencing the child issuer. The child
